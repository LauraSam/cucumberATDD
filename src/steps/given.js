...
defineSupportCode(({ Given }) => {
  ...
  /*  ------------------------------------------------------*/
   Given(/^start my flowr front-end$/, () => {
       /* const engine = browser.options.desiredCapabilities.browserName;
        const mac = `test_e2e_mac_address_${engine}`;

        browser.url(`${browser.options.baseUrl}?mac=${mac}_${ScanarioName}`);
        browser.waitForVisible('#admin_digits');*/
    });
});
